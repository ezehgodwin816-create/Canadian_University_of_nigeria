...
Deno.serve(async req => {
  ...
  try {
    ...
    const feeRecordId = body?.fee_record_id ? String(body.fee_record_id).trim() : null

    if (!feeRecordId) {
      return response({ error: 'fee_record_id is required' }, 400)
    }
    ...
    if (['paid', 'settled', 'cancelled', 'waived'].includes(status)) {
